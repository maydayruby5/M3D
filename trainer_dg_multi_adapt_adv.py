...
from PointDA.Models_3 import DGCNN
from PointDA.Models_3 import DGCLIP, Discriminator
...
parser.add_argument('--exp_name', type=str, default='model2shape_clip_0711',  help='Name of the experiment')
parser.add_argument('--out_path', type=str, default='./experiments', help='log folder path')
parser.add_argument('--dataroot', type=str, default='./data', metavar='N', help='data path')
parser.add_argument('--src_dataset1', type=str, default='modelnet', choices=['scannet', 'shapenet', 'scannet'])
parser.add_argument('--src_dataset2', type=str, default='scannet', choices=['scannet', 'shapenet', 'scannet'])
...
ad_net = Discriminator().to(device)

# Handle multi-gpu
if (device.type == 'cuda') and len(args.gpus) > 1:
    model = nn.DataParallel(model, args.gpus)
    ad_net = nn.DataParallel(ad_net, args.gpus)
...
opt = optim.SGD(itertools.chain(model.parameters(),ad_net.parameters()), lr=args.lr, momentum=args.momentum, weight_decay=args.wd) if args.optimizer == "SGD" \
    else optim.Adam(itertools.chain(model.parameters(),ad_net.parameters()), lr=args.lr, weight_decay=args.wd)
...
# Train
# ==================
src_best_val_acc = trgt_best_val_acc = best_val_epoch = trgt_best_test_acc = 0
src_best_val_loss = trgt_best_val_loss = MAX_LOSS
best_model = io.save_model(model)

def AdLoss(discriminator_out, criterion, bs):
    # generate ad_label
    ad_label1_index = torch.LongTensor(bs, 1).fill_(0)
    ad_label1 = ad_label1_index.cuda()
    ad_label2_index = torch.LongTensor(bs, 1).fill_(1)
...
for epoch in range(args.epochs):
    ...
    src_print_losses = {"total1": 0.0, "mixup1": 0.0, "total2": 0.0, "mixup2":0.0, "adv":0.0, "adv_total":0.0, "DefRec_adv":0.0}
    if args.DefRec_on_src:
        src_print_losses['DefRec1'] = 0.0
        src_print_losses['DefRec2'] = 0.0
    ...
    batch_idx = 1
    for data1, data2 in zip(src_train_loader1, src_train_loader2):
        opt.zero_grad()

        #### source data ####
        if data1 is not None:
            src_data1, src_label1 = data1[0].to(device), data1[1].to(device).squeeze()
            # change to [batch_size, num_coordinates, num_points]
            src_data = src_data1
            src_data = src_data.permute(0, 2, 1)
            src_label = src_label1
            ...
            if args.DefRec_on_src:
                ...
                src_logits, feat1 = model(src_data, activate_DefRec=True)
                loss = DefRec.calc_loss(args, src_logits, src_data_orig, src_mask)
                src_print_losses['DefRec1'] += loss.item() * batch_size
                src_print_losses['total1'] += loss.item() * batch_size
                loss.backward()

            if args.apply_PCM:
                src_data = src_data_orig.clone()
                src_data, mixup_vals = PCM.mix_shapes(args, src_data, src_label)
                src_cls_logits, feat1 = model(src_data, activate_DefRec=False)
                loss = PCM.calc_loss(args, src_cls_logits, mixup_vals, criterion)
                src_print_losses['mixup1'] += loss.item() * batch_size
                src_print_losses['total1'] += loss.item() * batch_size
                loss.backward()
                ...
                src_cls_logits, feat1 = model(src_data, activate_DefRec=True)
        ...
        if data2 is not None:
            src_data2, src_label2 = data2[0].to(device), data2[1].to(device).squeeze()
            # change to [batch_size, num_coordinates, num_points]
            src_data = src_data2
            src_data = src_data.permute(0, 2, 1)
            src_label = src_label2
            batch_size = src_data.size()[0]
            src_data_orig = src_data.clone()
            device = torch.device("cuda:" + str(src_data.get_device()) if args.cuda else "cpu")
            ...
            if args.DefRec_on_src:
                src_data, src_mask = DefRec.deform_input(src_data, lookup, args.DefRec_dist, device)
                src_logits, feat2 = model(src_data, activate_DefRec=True)
                loss = DefRec.calc_loss(args, src_logits, src_data_orig, src_mask)
                src_print_losses['DefRec2'] += loss.item() * batch_size
                src_print_losses['total2'] += loss.item() * batch_size
                loss.backward()

            if args.apply_PCM:
                src_data = src_data_orig.clone()
                src_data, mixup_vals = PCM.mix_shapes(args, src_data, src_label)
                src_cls_logits, feat2 = model(src_data, activate_DefRec=False)
                loss = PCM.calc_loss(args, src_cls_logits, mixup_vals, criterion)
                src_print_losses['mixup2'] += loss.item() * batch_size
                src_print_losses['total2'] += loss.item() * batch_size
                loss.backward()
                ...
                src_data = src_data_orig.clone()
                # predict with undistorted shape
                src_cls_logits, feat2 = model(src_data, activate_DefRec=True)
                src_feat2 = feat2
                loss = (1 - args.DefRec_weight) * criterion(src_cls_logits["cls"], src_label)
                src_print_losses['mixup2'] += loss.item() * batch_size
                src_print_losses['total2'] += loss.item() * batch_size
        ...
        if data1 and data2 is not None:
            ...
            src_data, src_mask = DefRec.deform_input(src_data, lookup, args.DefRec_dist, device)
            src_logits, src_fea = model(src_data, activate_DefRec=True)
            #src_defrec_loss = DefRec.calc_loss(args, src_logits, src_data_orig, src_mask)

            target_data, target_mask = DefRec.deform_input(trgt_data, lookup, args.DefRec_dist, device)
            target_logits, target_fea = model(target_data, activate_DefRec=True)
            #trgt_defrec_loss = DefRec.calc_loss(args, target_logits, trgt_data_orig, target_mask)

            #defrec_loss =  trgt_defrec_loss + src_defrec_loss
            discriminator_out = ad_net(torch.cat((src_fea, target_fea), 0))

            adloss = AdLoss(discriminator_out, criterion, args.batch_size)  # 计算出ad损失
            loss = adloss * 10*args.adv_weight

            src_print_losses['adv'] += loss.item() * batch_size * 2  # 目标域的对抗损失
            #src_print_losses['DefRec_adv'] += defrec_loss.item() * batch_size * 2
            #src_print_losses['adv_total'] += loss.item() * batch_size * 2
    ...
    # Validation
    #===================
    src_val_acc1, src_val_loss1, src_conf_mat1 = test(src_val_loader1, model, "Source1", "Val1", epoch)
    src_val_acc2, src_val_loss2, src_conf_mat2 = test(src_val_loader2, model, "Source2", "Val2", epoch)
    ...
    if trgt_test_acc > trgt_best_test_acc:
        src_best_val_acc1 = src_val_acc1
        src_best_val_loss1 = src_val_loss1
        src_best_val_acc2 = src_val_acc2
        src_best_val_loss2 = src_val_loss2
...
io.cprint("Best model was found at epoch %d, source validation accuracy: %.4f, source validation loss: %.4f,"
          "target validation accuracy: %.4f, target validation loss: %.4f，target test accuracy: %.4f"
          % (best_val_epoch, src_best_val_acc, src_best_val_loss, trgt_best_val_acc, trgt_best_val_loss, trgt_best_test_acc))
