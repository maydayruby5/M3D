...
def distribute(depth, _x, _y, size_x, size_y, image_height, image_width):
    ...
    of size (size_x, size_y).将与每个点关联的深度分布到区域中的离散坐标
    :param depth:
    :param _x:
    :param _y:
    :param size_x:
...
def points2depth(points, image_height, image_width, size_x=4, size_y=4):
    ...
    weighed_value_scattered, weight_scattered = distribute( #将与每个点关联的深度分布到区域中的离散坐标
        depth=depth,
        _x=_x,
        _y=_y,
        size_x=size_x,
...
#这个用在distribute_img_fea_points，等于也没用
...
class PCViews:
    ...
    def __init__(self):
        ...
        self.rot_mat = euler2mat(angle).transpose(1, 2)#Convert euler angles to rotation matrix
