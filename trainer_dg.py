...
from PointDA.Models_2 import DGCNN
...
parser.add_argument('--src_dataset1', type=str, default='scannet', choices=['scannet', 'shapenet', 'scannet'])
...
parser.add_argument('--model', type=str, default='dgcnn', choices=['pointnet', 'dgcnn','dgclip'], help='Model to use')
...
def test(test_loader, model=None, set_type="Target", partition="Val", epoch=0):
    ...
    # 输出结果
    print(f"程序开始时间: {start_time} (Unix时间戳)")
    print(f"程序结束时间: {end_time} (Unix时间戳)")
    print(f"每个样本时长: {execution_time:.6f} 毫秒")  # 保留6位小数
...
# Train
# ==================
src_best_val_acc = trgt_best_val_acc = best_val_epoch = trgt_best_test_acc =best_val_tgr_epoch= 0
trgt_best_test_acc1=0
...
for epoch in range(args.epochs):
    ...
    batch_idx = 1
    for data1, data2 in zip(src_train_loader1, trgt_train_loader):
        opt.zero_grad()
        ...
        if data1  is not None:
            ...
            #src_data = torch.cat([src_data1,src_data2], 0)
            src_data = src_data.permute(0, 2, 1)
            #src_label = torch.cat([src_label1,src_label2], 0)
            ...
            if args.apply_PCM:
                ...
                loss = PCM.calc_loss(args, src_cls_logits['cls'], mixup_vals, criterion)
    ...
    print(f"程序开始时间: {start_time} (Unix时间戳)")
    print(f"程序结束时间: {end_time} (Unix时间戳)")
    execution_time = (end_time - start_time)*1000/6110
    print(f"程序运行时长: {execution_time:.6f} 毫秒")
    #===================
    # Validation
    #===================
    src_val_acc1, src_val_loss1, src_conf_mat1 = test(src_val_loader1, model, "Source", "Val", epoch)
    ...
    trgt_test_acc, trgt_test_loss, trgt_conf_mat = test(trgt_test_loader, model, "Target", "Test", epoch)
    #io.cprint('\n' + str(trgt_conf_mat))

    # save model according to best source model (since we don't have target labels)
    if src_val_acc1 > src_best_val_acc:
        src_best_val_acc = src_val_acc1
        src_best_val_loss = src_val_loss1
        ...
        best_model = io.save_model(model)
    print(f"目前按源域目标域最高准确率:{trgt_best_test_acc}，在第{best_val_epoch}轮")
    if trgt_best_test_acc1 < trgt_test_acc:
        trgt_best_test_acc1 = trgt_test_acc

        best_val_tgr_epoch = epoch
        best_val_tgr_epoch_mat = trgt_conf_mat
        best_tgrmodel = io.save_model(model)
    print(f"目前按目标域最高准确率:{trgt_best_test_acc1}，在第{best_val_tgr_epoch}轮")
